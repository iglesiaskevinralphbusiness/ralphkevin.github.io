<?php
    ...
    //select manga
    $sql = "SELECT * FROM categories WHERE url = '$url' GROUP BY manga_id DESC LIMIT $page, $item_per_page";
    $result_active = $conn->query($sql);
    while($row_active = $result_active->fetch_assoc()) {
        
        $active_manga_id = $row_active['manga_id'];

        $sql = "SELECT * FROM manga WHERE id = '$active_manga_id'";
    ...
    //select manga pagination
    $sql = "SELECT * FROM categories WHERE url = '$url' GROUP BY manga_id";
