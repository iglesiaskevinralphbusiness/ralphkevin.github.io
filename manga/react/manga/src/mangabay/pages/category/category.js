...
class LatestManga extends React.Component {
  ...
  render() {
    ...
    const popularUpdates = this.getPopularUpdates(list, chapters, category);
    const latestRelease = this.getLatestRelease(list);
    const fbCommentUrl = "/popular/" + category;
  ...
  getPopularUpdates = (list, chapters, category) => {
    ...
    const organizeByCategory = organize.filter(o => {
      const find = o.genre.find(g => g.name.toUpperCase() == category.toUpperCase());
      console.log(find);
      if(find){
        return true;
      }
